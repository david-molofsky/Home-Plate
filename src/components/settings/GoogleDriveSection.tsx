...
export function GoogleDriveSection() {
  ...
  const handleAutoBackupToggle = (checked: boolean) => {
    if (checked) setAutoBackupConfirmOpen(true);
    else setAutoBackupEnabled(false);
  };
  ...
  const handleDisconnect = () => run(async () => { await signOutOfDrive(); setStatus(null); });
  ...
  const handleOpenImport = async () => {
    ...
    setDriveFiles([]);
    try {
      setDriveFiles(await listDriveExports());
    } catch (err) {
      setStatus({ type: 'error', message: err instanceof Error ? err.message : 'Could not list Drive files.' });
      setImportOpen(false);
    } finally {
  ...
  return (
    <CollapsibleSection title="Google Drive" icon={GoogleIcon}>
      {!connected ? (
        ...
        <>
          ...
          <Stack direction="row" spacing={1.5} flexWrap="wrap" useFlexGap>
            ...
            <Button variant="outlined" startIcon={<CloudDownloadOutlinedIcon />} onClick={handleOpenImport} disabled={busy}>
              Import from Drive
            </Button>
            <Button startIcon={<LogoutOutlinedIcon />} onClick={handleDisconnect} disabled={busy} color="inherit" size="small" sx={{ ml: 'auto' }}>
              Disconnect
            </Button>
          </Stack>
            ...
            <Stack direction="row" alignItems="flex-start" justifyContent="space-between" spacing={2}>
              <Box>
                ...
                <Typography variant="body2" color="text.secondary" sx={{ mt: 0.5 }}>
                  Backs up to Drive at 23:59 each day this device is on, or as soon as it's next opened.
                </Typography>
              </Box>
              <FormControlLabel
                sx={{ m: 0 }}
                control={<Switch checked={autoBackupEnabled} onChange={(e) => handleAutoBackupToggle(e.target.checked)} />}
                label=""
              />
            </Stack>

            <Alert severity="warning" sx={{ mt: 1.5 }}>
              Only enable this on one household device. Turning it on elsewhere too can cause
              backups to overwrite each other unpredictably.
            </Alert>

            <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mt: 1 }}>
              Last automatic backup: {lastAutoBackupAt ? dayjs(lastAutoBackupAt).format('D MMM YYYY, HH:mm') : 'never'}
            </Typography>
          </Box>
        </>
      )}
      ...
      <Dialog open={autoBackupConfirmOpen} onClose={() => setAutoBackupConfirmOpen(false)} fullWidth maxWidth="xs">
        ...
        <DialogContent>
          ...
          <Alert severity="warning">Enable this on one device only, to avoid backups overwriting each other.</Alert>
        </DialogContent>
        <DialogActions>
